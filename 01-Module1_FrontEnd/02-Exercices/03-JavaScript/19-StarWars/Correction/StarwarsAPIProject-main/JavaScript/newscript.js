...
async function loadNavLabels(label) {
  ...
  const data = await fetch(`${url}${label}`)
    .then((res) => res.json())
    .catch(function (error) {
      console.log(error);
    });
  ...
  if (data.next) {
    ...
    nextBtn.addEventListener("click", () =>
      loadNavLabels(nextBtn.getAttribute("datalink"))
    );
  ...
  if (data.previous) {
    ...
    prevBtn.addEventListener("click", () =>
      loadNavLabels(prevBtn.getAttribute("datalink"))
    );
...
async function loadFullCard(label) {
  ...
  if (splitedUrl[4] == "films") {
    document.body.style =
      "background-image: url(https://lumiere-a.akamaihd.net/v1/images/image_1760b382.jpeg?region=0,0,2048,878);";
    if (
      screen.width < 600 &&
      document.querySelector("nav").getAttribute("class") != "hide"
    ) {
  ...
  if (splitedUrl[4] == "species") {
    document.body.style =
      " background-image: url(https://www.10wallpaper.com/wallpaper/1920x1080/1512/Star_Wars_The_Force_Awakens_2015_HD_Wallpaper_07_1920x1080.jpg)";
    if (
      screen.width < 600 &&
      document.querySelector("nav").getAttribute("class") != "hide"
    ) {
  ...
  if (splitedUrl[4] == "starships") {
    ...
    document.body.style =
      "background-image: url(https://wallpaperaccess.com/full/2506528.jpg)";
    if (
      screen.width < 600 &&
      document.querySelector("nav").getAttribute("class") != "hide"
    ) {
  ...
  if (splitedUrl[4] == "planets") {
    document.body.style =
      " background-image: url(https://img.wallpapersafari.com/desktop/1920/1080/78/46/OFdp36.jpg";
    if (
      screen.width < 600 &&
      document.querySelector("nav").getAttribute("class") != "hide"
    ) {
  ...
  if (splitedUrl[4] == "vehicles") {
    ...
    document.body.style =
      " background-image: url(https://img5.goodfon.com/original/1728x972/d/2a/andrew-march-by-andrew-march-mantis-in-trouble-chris-kuhn-ar.jpg)";
    if (
      screen.width < 600 &&
      document.querySelector("nav").getAttribute("class") != "hide"
    ) {
...
async function loadMoviePicture(datapic) {
  ...
  const empireStrike =
    "https://images-na.ssl-images-amazon.com/images/S/pv-target-images/36130eca001baf033aaff6778b21abf5bcfa0d16f944074c07e5e28da7f792bc._RI_V_TTW_.jpg";
  const returnOfjedi =
    "https://images-na.ssl-images-amazon.com/images/I/81g8vEs4ixL.jpg";
  const phantomMenace =
    "https://www.cinemaffiche.fr/4015-tm_thickbox_default/star-wars-la-menace-fantome-episode-1-star-wars-1-the-phantom-menace.jpg";
  const attackOfClones =
    "https://images-na.ssl-images-amazon.com/images/I/91bvd0Yxd4L.jpg";
  const sithRevenge =
    "https://fr.web.img3.acsta.net/medias/nmedia/18/35/53/23/18423997.jpg";
...
async function createInfoBtn(parameter) {
  const data = await fetch(`${parameter}`)
    .then((res) => res.json())
    .catch(function (error) {
      console.log(error);
    });
  ...
  if (data.name) {
    ...
    vimg.setAttribute(
      "src",
      "https://cdn0.iconfinder.com/data/icons/star-wars-3/154/droid-helmet-soldier-star-wars-256.png"
    );
  ...
  if (data.title) {
    ...
    vimg.setAttribute(
      "src",
      "https://as1.ftcdn.net/v2/jpg/02/05/18/44/1000_F_205184418_t91TINxilW8CT2aWEqVW9J8b6CKf8iss.jpg"
    );
    vimg.setAttribute("class", "vehicleimg");
    newBtn.appendChild(vimg);
  }
...
async function createPlanetsBtn(parameter) {
  ...
  if (data.name) {
    ...
    vimg.setAttribute(
      "src",
      "https://cdn4.iconfinder.com/data/icons/star-wars-13/50/30-512.png"
    );
    vimg.setAttribute("class", "vehicleimg");
    newBtn.appendChild(vimg);
  }
...
async function createVehicleBtn(parameter) {
  ...
  vimg.setAttribute(
    "src",
    "https://cdn2.iconfinder.com/data/icons/star-wars-6/24/AT-AT-Walker-512.png"
  );
  vimg.setAttribute("class", "vehicleimg");
  newBtn.appendChild(vimg);
}
...
async function searchIt(param) {
  const datasearch = await fetch(`${urlsearch}${param}`)
    .then((res) => res.json())
    .catch(function (error) {
      console.log(error);
    });
